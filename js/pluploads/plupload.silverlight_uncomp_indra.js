		...
		init: function (p, q) {
			...
			p.bind("Silverlight:Init", function () {
				...
				p.bind("Silverlight:UploadChunkError", function (s, v, t, w, u) {
					p.trigger("Error", {
						code: d.IO_ERROR,
						message: "IO Error.O Error.,details:u,file:s.getFile(r[v])})});p.bind(",
						details: u,
						file: s.getFile(r[v])
					})
				});
			...
			,
				function (s) {
					...
					if (t) {
						...
						v = d.getSize(t);
						d.extend(b.getElementById(s.id + "_silverlight_container"
						).style, {
							top: u.y + "px",
							left: u.x + "px",
							width: v.w + "px",
					...
					function (s) {
						...
						);if(t){k.removeChild(t)}});q({success:true})})}})})(window,document,plupload);