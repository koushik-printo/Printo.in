...
$.widget('ui.spinner', {
	...
	_createButtons: function(input) {
		...
		wrapper.append(btnContainer.css({ height: height, left: -buttonWidth-rightMargin,
			...
			top: (input.offset().top - (wrapper.offset().top + 18 )) + 'px' })
		...
		buttons.css({ width: buttonWidth - (box ? buttons.outerWidth() - buttons.width() : 0), height: height - (box ? buttons.outerHeight() - buttons.height() : 0) + 3 });
		upButton = buttons[0];
		downButton = buttons[1];

		// fix icon centering
		icons = buttons.find('.ui-icon');
		icons.css({ marginLeft: (buttons.innerWidth() - icons.width()) / 2, marginTop:  (buttons.innerHeight() - icons.height()) / 3 });
